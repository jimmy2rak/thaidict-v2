...
2. **文档 20 表的"前端入口"几乎全部已存在**：扫描后，除下面 2 项外，其余 18 张表在新系统里**既有 db 层函数、也有对应前端页面**，无需新增前端入口。
   ...
   - `dictionary`（旧版表）：已被 `dictionary_full` 取代，真实模式代码不再引用，未建该表。
...
|---|--------|---------------|----------------------|------|
| 1 | `dictionary_full` | `db/search.js` · `AppContext.jsx`(loadDictFromDB) | 首页搜索、词条详情 `WordDetailPage`、单词本查词 | ✅ 已覆盖 |
| 2 | `dictionary`（旧版） | — | — | ⚪ **已并入 `dictionary_full`**（real 模式 `getDictionaryCount` 查 dictionary_full），不单独建表 |
...
### 3.1 真正的"前端缺口"：无
文档 20 表中，**18 张表均已具备前端入口**；剩余 2 张（`system_config` 后端配置、`dictionary` 旧表）按架构本就不需要前端入口。
