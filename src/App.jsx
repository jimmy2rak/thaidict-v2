    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', height: '100%', overflow: 'hidden' }}>
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        {app.visitedPages.has('home') && (
          <div style={{ display: app.page === 'home' ? 'block' : 'none', height: '100%' }}>
            <HomePage />
          </div>
        )}
        {app.visitedPages.has('words') && (
          <div style={{ display: app.page === 'words' ? 'block' : 'none', height: '100%' }}>
            <WordBookPage />
          </div>
        )}
        {app.visitedPages.has('learn') && (
          <div style={{ display: app.page === 'learn' ? 'block' : 'none', height: '100%' }}>
            <LearnPage />
          </div>
        )}
        {app.visitedPages.has('me') && (
          <div style={{ display: app.page === 'me' ? 'block' : 'none', height: '100%' }}>
            <ProfilePage />
          </div>
        )}
